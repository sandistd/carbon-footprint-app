...
export default function Create({ auth, emissionFactors, stakeholders }: Props) {
  ...
  useEffect(() => {
    if (selectedFactor && data.activity_value) {
      ...
      const recValue = data.rec_value ? parseFloat(data.rec_value) : 0;

      if (!isNaN(activityValue)) {
        const effectiveValue = activityValue - recValue;
        const result = (effectiveValue * selectedFactor.factor) / 1000;
        setEmissionPreview(Math.max(result, 0));
      } else {
  ...
  }, [selectedFactor, data.activity_value, data.rec_value]);
  ...
  return (
    <AppLayout>
      <Head title="Tambah Emisi Scope 2" />
      ...
      <div className="py-12">
        <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-3xl font-bold tracking-tight">Tambah Emisi Scope 2</h2>
              <p className="text-muted-foreground mt-2">
                Input data emisi tidak langsung dari konsumsi energi yang dibeli
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Lengkapi formulir berikut untuk menambah data emisi Scope 2
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="rec_value">
                    Renewable Energy Certificate (REC) - KWh
                  </Label>
                  <Input
                    id="rec_value"
                    type="number"
                    step="0.01"
                    placeholder="Masukkan nilai REC jika ada (opsional)"
                    value={data.rec_value}
                    onChange={(e) => setData('rec_value', e.target.value)}
                  />
                  <p className="text-xs text-muted-foreground">
                    REC akan mengurangi total emisi dari konsumsi listrik
                  </p>
                  {errors.rec_value && (
                    <p className="text-sm text-destructive">{errors.rec_value}</p>
                  )}
                </div>
                    ...
                    <AlertDescription>
                      <div className="space-y-2">
                        ...
                        <div className="text-sm space-y-1">
                          ...
                          {data.rec_value && parseFloat(data.rec_value) > 0 && (
                            <div>
                              REC: {formatNumber(parseFloat(data.rec_value))}{' '}
                              {selectedFactor.unit.split('/')[0]}
                            </div>
                          )}
